...
var orderModels = {
    ...
    orderList: function (data, callback) {
        ...
        aggregate.group({
            ...
            // },
            teamMemberOrder: {
                $addToSet: "$teamMemberOrder" //{$arrayElemAt:["$teamMemberOrder",0]}
            },
                ...
                async.forEach(results, function (item, callback) {
                    if (item.teamOrderUserList.length > 0) {
                        ...
                        if (item.teamMemberOrder != null && item.teamMemberOrder.length > 0) {
                            async.forEach(item.teamMemberOrder, function (order, callback) {
                        ...
                        // if (item.childOrderIds != null && item.childOrderIds.length > 0) {
                        //     let order_details = '';

                        //     orderModels.teamOrderList(item.childOrderIds, function (response) {
                        //         //console.log("response.response_data=======",response);
                        //         if (response.response_code == 2000) {

                        //             order_details = response.response_data;
                        //             item.teamMemberOrder = response.response_data;

                        //             //console.log("item.teamMemberOrder",item.teamMemberOrder);
                        //         }
    ...
    let promise = new Promise(function (resolve, reject) {
            ...
            checkSettingResponse.map(async function (data) {
                ...
                if (data.response_code == '2000') {
                    ...
                    let earnClover = parseInt(transactionAmount / discount) * parseFloat(clover)
